...
OD_extension_t OD_variableInt8_extension = {
    .object = NULL,
                                  ...
                                  uint32_t *errInfo)
{
    ...
    OD_extension_init(OD_ENTRY_H2000_channel_Trigger,
                      &OD_variableInt8_extension);
    ...
    err = objectAccessOD_init(&objectAccessOD,
                              OD_ENTRY_H2000_channel_Trigger,
                              errInfo);
    if (err != CO_ERROR_NO)
        return err;

    domainDemo_init(OD_ENTRY_H2000_channel_Trigger, errInfo);
    return err;
}
...
void app_programRt(CO_t *co, uint32_t timer1usDiff) {
    ...
    int8_t value_current = OD_RAM.x2000_channel_Trigger.channel_01_Trigger;
    printf("Hello World from CAnopenNode - this is Rt-Task");
    ...
    if (value_current != value_old) {
        OD_requestTPDO(OD_ENTRY_H2001_COM_Channel_01, 3); /* subindex is 3 */
    }
